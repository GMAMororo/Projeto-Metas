...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchEventos(){
        try{
            const res = await fetch('actions/events.php'); // Certifique-se que o caminho está certo (ex: php/events.php se estiver em pasta)
    ...
    async function salvarEvento(payload){
        try {
            ...
            await fetch('actions/add_event.php', {method:'POST', body:formData});
    ...
    async function atualizarEvento(payload){
        try {
            const formData = new FormData();
            Object.keys(payload).forEach(k=> formData.append(k,payload[k]));
            await fetch('actions/update_event.php', {method:'POST', body:formData}); // Se não tiver update_event, use add_event se ele suportar UPDATE
        } catch(e) { console.error(e); }
    }
    ...
    async function deletarEvento(id){
        try {
            ...
            const res = await fetch('actions/delete_event.php', {method:'POST', body:formData});
            const data = await res.json();
